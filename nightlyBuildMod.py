...
for p in NBuserConfig.PROJECTS:
  ...
  #---------------------------------------------------------------------
  # Loop over each vpath directory
  #---------------------------------------------------------------------
  kount = 0
  for v in NBuserConfig.VPATH_DIRS:
    NBlogMessages.writeMessage( v )
    #---------------------------------------------------------------------
    # connect to the project directory
    #---------------------------------------------------------------------
    os.chdir( projectBaseDir)
    vpathDir = os.path.join(projectBaseDir, v)
    print vpathDir
    #---------------------------------------------------------------------
    # if the vpath directory is there, delete it and create a new one
    # otherwise create it
    #---------------------------------------------------------------------
    if os.path.isdir( vpathDir) :
      distutils.dir_util.remove_tree( vpathDir)
      os.mkdir( vpathDir)
    else :
      os.mkdir( vpathDir)
    #---------------------------------------------------------------------
    # connect to the vpath directory
    #---------------------------------------------------------------------
    os.chdir( vpathDir)
    
    #---------------------------------------------------------------------
    # run configure part of build
    # first get the configure options
    #---------------------------------------------------------------------

    configCmd = os.path.join(projectCheckOutDir, NBuserConfig.VPATH_CONFIG_ARGS[ kount])
    kount = kount + 1
