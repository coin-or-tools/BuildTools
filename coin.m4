...
AC_DEFUN([AC_COIN_CHECK_HEADER],[AC_CHECK_HEADERS([$1],[$2],[$3],[$4])])

###########################################################################
#                       COIN_CHECK_CXX_CHEADER                            #
###########################################################################

# This macro checks for C header files that are used from C++.  For a given
...
AC_DEFUN([AC_COIN_F77_WRAPPERS],[AC_REQUIRE([AC_F77_WRAPPERS])])
...
# ToDo for now, do not reuse libtool script because code below may need some updates
#   e.g., getting the SED and ECHO variables
LIBTOOL=
...
'
  ...
  # And some defines (assuming here that the packages base dir doesn't have a config.h file)
...
# If libtool exists in the directory higher up, we use that one
...
if test x$coin_cxx_is_cl != xtrue && test x$coin_cc_is_cl != xtrue ; then
