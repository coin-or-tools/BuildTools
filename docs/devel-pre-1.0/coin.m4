...
# --enable-doscompile option which is to be used when one wants to build an
# executable under Cygwin which also runs directly under DOS (without requiring
# Cygwin1.dll). Using --enable-doscompile=msvc will direct configure to use cl
# and link as the compiler and linker, respectively.  Using --enable-doscompile
# =mingw will use GCC tools and add the -mno-cygwin flag.  Specifying a bare
# --enable-doscompile is the same as choosing mingw.
...
Set to msvc to use cl & link, instead of gcc/g++/ld.
Set to mingw to use gcc/g++/ld with -mno-cygwin.
Default when mentioned: mingw. Default when not mentioned: disabled.])],
...
# Choosing an msvc build under cygwin forces the cl compiler. Arguably one
# could include g++ as the second choice, but that amounts to changing on the
# fly to the mingw option.
...
# ToDo decide about unroll-loops
  ...
  if test -z "$coin_opt_cxxflags"; then
    case $build in
      *-cygwin* | *-mingw*)
        case "$CXX" in
	  # Warning 4996 points to versions of standard library functions that
	  # are `secure'. Generates a huge number of warnings that swamp other
	  # output.
          cl* | */cl*)
            coin_opt_cxxflags='-O2'
            coin_add_cxxflags='-nologo -EHsc -GR -MT -wd4996'
            coin_dbg_cxxflags='-Z7'
            ;;
        esac
        ;;
      *-linux-*)
...
# possible to provide additional -D flags in the variable CDEFS. See
# COIN_PROG_CXX for additional comments regarding the cl compiler.
...
AC_MSG_NOTICE([Fortran compiler candidates: $comps])
AC_PROG_F77($comps)
if test -z "$F77" ; then
...
[ AC_REQUIRE([AC_COIN_ENABLE_DOSCOMPILE])
  ...
  AC_CHECKING([for presence of a Fortran compiler.])
  AC_MSG_NOTICE([Fortran compiler candidates: $comps.])
  AC_CHECK_PROGS([F77],[$comps],[unavailable])
...
# Somewhere down in the expansion of this macro, tests are made that involve
# the Fortran compiler. If F77 is unset, autoconf will make incorrect
# assumptions.

AC_DEFUN([AC_COIN_INIT_AUTO_TOOLS],
[AC_REQUIRE([AC_COIN_DISABLE_STATIC])
 AC_REQUIRE([AC_COIN_FIND_F77])
...
AC_MSG_NOTICE([Beginning libtool initialisation.])
...
# This macro checks for a library containing the BLAS library.  It tries
# standard libraries, and if none is found to be working, it checks whether the
# BLAS ThirdParty/Blas directory has been configured.  It adds to ADDLIBS any
# flags required to link with an externally provided BLAS.  It defines the
# makefile conditional and preprocessor macro COIN_HAS_BLAS, if BLAS is
# available, and it defines the makefile conditional COIN_BUILD_BLAS, if BLAS
# is compiled within COIN.

# This macro will trigger a hard failure of configure if basic operational
# tests fail for the Fortran compiler. The failure originates in autoconf
# boilerplate.
...
# This macro checks for a library containing the LAPACK library.  It tries
# standard libraries, and if none is found to be working, it checks whether the
# LAPACK ThirdParty/Lapack directory has been configured.  It adds to ADDLIBS
# any flags required to link with an externally provided LAPACK.  It defines
# the makefile conditional and preprocessor macro COIN_HAS_LAPACK, if lapack is
# available, and it defines the makefile conditional COIN_BUILD_LAPACK, if
# lapack is compiled within COIN.
