 ...
 1. All dependent projects are treated more or less as "third-party", so that it is now easy to link against installed version of COIN-OR projects (using pkg-config if it is available). This also means that what were truly "third-party" projects are now treated more or less the same as COIN-OR projects. We provide build, pkg-config, and install support for these projects if source is available and otherwise provide a mechanism for linking to installed libraries, as before.
 ...
 * `Root/Xxx/doxydoc/doxygen.conf.in`
...
## `Root/Xxx/configure.ac`
 ...
 1. The list of files to be created by the configure script (listed as arguments to the `AC_CONFIG_FILES` command must now include `xxx.pc` and `xxx-uninstalled.pc` (see below for a description of the corresponding .in files).


## `Root/Xxx/Makefile.am`

The main change here is that we are now replacing the old `xxx_addlibs.txt` file with `xxx.pc` and `xxx-uninstalled.pc` files (see below).
...
if COIN_CXX_IS_CL
       echo "/libpath:`$(CYGPATH_W) `@`abs_lib_dir`@`` libxxx.lib `@`XXXLIB_LIBS_INSTALLED`@`" > $(addlibsdir)/xxx_addlibs.txt
else
       echo -L`@`abs_lib_dir`@` -lxxx `@`XXXLIB_LIBS_INSTALLED`@` > $(addlibsdir)/xxx_addlibs.txt
endif
endif 
...
This is necessary to setup the DEPENDENCIES variable in the Makefile's that use project Xxx.
```
prefix=`@`prefix`@`
...
Description: Xxx does nothing useful
Version: `@`PACKAGE_VERSION`@`
Libs: ${libdir}/libXxx.la `@`XXXLIB_PCLIBS`@`
...
In the latter case, the AC_COIN_CHECK_PACKAGE macro should have setup a variable XXXLIB_CFLAGS_INSTALLED.
```
ifeq ($(COIN_HAS_PKGCONFIG), TRUE)
  INCL = `PKG_CONFIG_PATH=`@`COIN_PKG_CONFIG_PATH`@` `@`PKG_CONFIG`@` --cflags xxx`
else
  INCL = `@`XXXLIB_CFLAGS_INSTALLED`@`
endif
INCL += $(ADDINCFLAGS)
```
...
If it is not available, then AC_COIN_CHECK_PACKAGE has setup a variable XXXLIB_LIBS_INSTALLED that contains all dependencies of our project.
...
For the XXXLIB_LIBS_INSTALLED variable, configure has already done this.
```
# Linker flags
ifeq ($(COIN_HAS_PKGCONFIG), TRUE)
  LIBS = `PKG_CONFIG_PATH=`@`COIN_PKG_CONFIG_PATH`@` `@`PKG_CONFIG`@` --libs xxx`
else
  ifeq ($(COIN_CXX_IS_CL), TRUE)
    LIBS = /link /libpath:`$(CYGPATH_W) `@`abs_lib_dir`@`` libXxx.lib `@`XXXLIB_LIBS_INSTALLED`@`
  else
    LIBS = -L`@`abs_lib_dir`@` -lXxx `@`XXXLIB_LIBS_INSTALLED`@`
  endif
endif
```
...
See [here](https://projects.coin-or.org/BuildTools/wiki/pm-doxygen).
